...
echo "Push to $GITHUB_REPOSITORY in branch $INITIATIVE_NAME"
cd "$CLONE_DIR"
git add .
...
curl --location --request POST "https://api.github.com/repos/$DEST_GITHUB_USERNAME/$DEST_GITHUB_REPO/pulls?access_token=$API_TOKEN_GITHUB" \
--header 'Accept: application/vnd.github.v3+json' \
--header 'Content-Type: application/json' \
--data-raw "{
    \"title\": \"$SYSTEM_NAME at $(date)\",
    \"head\": \"$INITIATIVE_NAME\",
    \"base\": \"master\"
}"
